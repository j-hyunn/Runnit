...
abstract class AppUser with _$AppUser {
  ...
  const factory AppUser({
    ...
    /// **주간 목표 거리(km).** AC-02에서 사용자가 직접 편집하는 값이며,
    /// 미설정이면 null(= 목표 없음 → UI는 목표 진행률 위젯을 숨긴다).
    ///
    /// ⚠️ 다른 두 개념과 혼동 금지:
    /// - **GM-05 주간 스트릭**: "주 N회 이상 뛰었는가"의 연속성 판정으로,
    ///   서버가 `profiles.current_streak_weeks` 등에 관리하는 읽기 전용 캐시다.
    ///   이 필드는 스트릭 판정에 **전혀 쓰이지 않는다**.
    /// - **TR-11 러닝별 목표**: 한 세션 시작 전에 잡는 목표(거리/시간)로,
    ///   러닝 세션에 종속된 일회성 값이다. 프로필에 저장하지 않는다.
    ///
    /// 즉 이 값은 "이번 주에 얼마나 뛰고 싶은가"라는 **개인 설정**일 뿐이고,
    /// 티어·랭킹·뱃지 판정에는 어떤 형태로도 입력되지 않는다. 단위가 km인 것도
    /// 의도적이다 — 사용자가 입력·확인하는 값이라 표시 단위와 같게 두었다
    /// (거리 캐시 필드들이 미터인 것과 다르다). DB 컬럼은 `profiles.weekly_goal_km`.
    double? weeklyGoalKm,
