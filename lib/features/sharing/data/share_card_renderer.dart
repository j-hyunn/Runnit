...
///
/// ## 투명 배경 (2026-08-26)
/// `ui.ImageByteFormat.png`는 알파 채널을 그대로 보존한다. 따라서 **위젯 트리
/// 어디에도 불투명 배경을 칠하지 않는 것**만으로 투명 PNG가 나온다 —
/// [ShareCardSurface]에 `ColoredBox`를 다시 넣는 순간 오버레이 스티커가 아니라
/// 사진을 가리는 판때기가 된다.
library;
...
/// 카드 출력 규격 — **9:16 투명 오버레이 스티커**.
///
/// 2026-08-26에 잠깐 16:9 가로 밴드로 바꿨다가, 사용자가 준 정확한 레퍼런스
/// 이미지(396×704 = 9:16 세로, 투명 배경)를 확인하고 **다시 9:16으로
/// 되돌렸다.** 인스타 스토리 캔버스와 같은 비율이라 스티커를 캔버스에
/// 그대로 앉혀도 자연스럽고, 확대해도 스토리 화면을 벗어나지 않는다.
///
/// ## 왜 1080×1920인가
/// 인스타 스토리 권장 해상도이자 대부분의 기기에서 업스케일 없이 선명한
/// 최소 크기다. 카드 대부분이 투명이라 PNG 용량은 불투명 이미지보다 작다.
