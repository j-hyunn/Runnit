...
| v0.24 | **R-12 회귀 가드**(2026-09-01, 마이그레이션 63 — 원래 62 로 작성·적용됐으나 병렬 세션의 `leaderboard_entries_active` 뷰가 62 를 선점해 파일을 63 으로 재번호, 원격 이력은 유지). 마이그레이션 57·61 이 `evaluate_badge_condition`(24KB 디스패처)을 **전문 재작성 없이 문자열 앵커 치환**으로 패치했다(57-5 `and le.finalized_at is not null`, 61-3 `runnit.badge_season` GUC). 이후 이 함수를 전문 재정의하는 마이그레이션이 두 조각을 다시 넣지 않으면 에러 없이 사라져 TRD #23·#30 이 부활한다. 가드는 라이브 함수 정의를 `pg_get_functiondef` 로 덤프해 두 문자열의 존재를 검사하고 없으면 `raise exception` 하는 `DO` 블록 — **이후 마이그레이션에서도 유지**한다. 근본 해법(전문 관리 또는 분기 함수 분해)은 이 함수를 다음에 실질적으로 손댈 때. QA `_workspace/20260901_qa_short-week-optionb.md` §3 P-15 |
| v0.23b | **마이그레이션 번호 충돌·드리프트 정리**(2026-09-02). 병렬 머지(PR #26 기능 / PR #27 회귀 가드 + 별도 세션 C-1 뷰)로 `62_` 파일이 2개가 됐다. 회귀 가드 파일을 `63_` 으로 재번호. C-1 세션이 원격에만 적용하고 파일화하지 않은 `62b_grant_season_calc_functions_for_active_view`(뷰의 `security_invoker` 경로가 부르는 season 계산 함수 4종 grant)를 사후 파일화 — `db reset`/브랜치 DB 정합. 본문 곳곳의 `마이그레이션 00~42` stale 표기를 `00~63` 으로 정정 |
...
**벤더 판별 시점** — 폰 단독 러닝은 세션 시작 시점부터 `phone` 확정. 워치는 HealthKit `sourceRevision.source.bundleIdentifier`/Health Connect `dataOrigin.packageName`에서 **지표가 도착하는 즉시**(실시간 경로) 또는 **워크아웃 임포트 시점**(Phase 4)에 확정한다. 판별 한계는 §8.4.1.
...
> 🛑 **아래 초안 DDL은 실제로 생성되지 않았다.** 라이브 스키마는 `supabase/migrations/00~64`이며, 주요 차이:
>
> | 초안 DDL | 라이브 스키마 |
> |---|---|
...
| 8 | Apple Watch 벤더 판별의 기기명 의존(§8.4.1) — `HKDevice`/`Metadata.device`를 얻는 얇은 platform channel이 필요한지 | Phase 4 웨어러블 실기기 테스트에서 오분류가 실제로 확인되면 |
| ~~9~~ | ~~`device_source_diversity_gte`의 OR 표현식 파싱 규약~~ → **해소(2026-08-26)**. 문법·매칭 시맨틱 모두 §3.1.2에 정본화 | — |
| ~~10~~ | ~~`runs.device_vendors` 마이그레이션(§4.0) 미적용~~ → **해소(2026-08-26, 마이그레이션 36)**. 컬럼·GIN 인덱스·백필·판정 로직 전부 적용됐다. 뱃지 5종은 이제 판정 가능하며, 실제 지급은 Phase 4 웨어러블 연동으로 `device_vendors` 에 값이 실리기 시작한 뒤부터다 | — |
...
| ~~27~~ | ~~서버가 샘플로 거리를 재계산하지 않는다~~ → **해소(2026-09-03, 마이그레이션 64 원격 적용 `xwtbwexcofcgmbvktwdo`)** · 🟡 **잔여 2건 추적 중**(아래 **잔여**). **라이브 검증 완료**: `runs.client_reported jsonb` 컬럼 생성 확인 / `recalc_run_from_samples(jsonb, activity_type)` volatility=stable · `authenticated` 실행 권한 유지(`public`·`anon`만 revoke, QA F-1 수정 정상) / `trg_runs_guard()` 는 트리거 함수라 전 롤 실행 권한 revoke / 방어 회귀(G-5) `[1,2]` · `[{},1]` · `'null'::jsonb` 모두 예외 없이 `applied=false` / 마이그레이션 63 회귀 가드 앵커 2개(`finalized_at is not null` · `runnit.badge_season`)가 라이브 `evaluate_badge_condition` 정의에 그대로 존재 / `runs` 트리거 5개 정상(`runs_guard` BEFORE + `runs_01_recompute_stats`·`runs_02_challenge_progress`·`runs_03_evaluate_badges`·`runs_04_notifications` AFTER) / security advisor 기존 6건 그대로(신규 0건). 절차·쿼리 전문은 `_workspace/20260903_backend_p0-validate-recalc-upload-guard.md` §1.4. 구현 내용: `recalc_run_from_samples(samples, activity_type)` 가 인접 구간을 다시 걸어 거리·이동시간을 산출하고, `trg_runs_guard` 가 파생값(페이스·`validate_run`·XP) 확정 **앞**에서 이를 호출한다. 상수는 전부 클라이언트 `GpsFilterConfig` 와 동일(25km/h 제거 · 0.6 m/s 미산입 · 구간 20초 상한). **덮어쓰기는 항상**(밴드 없음), **플래그는 서버 값 < 클라 주장 × 0.8 일 때만**, **늘리는 방향은 채택하지 않음**(`least(recalc, claimed)`) — 세 결정은 분리돼 있다(§7.1, 2026-09-03 사용자 확정). 원본 주장값은 `runs.client_reported` 에 최초 확정 때만 보존. **샘플 2개 미만·`indoor_run` 은 스킵**하고 클라이언트 값을 유지한다 — 이 취급의 최종 결정은 #26 ② 와 함께. 과거 행 백필은 하지 않았다(TI-08 · 트리거 폭발). HI-07 되돌리기 UPDATE 경로에서는 `is_flagged`/`flag_reason` 을 **재판정하지 않고 승계**한다 — 재계산발 `distance_mismatch` 는 복원된 old 상태에서 재현되지 않아, 제목 편집이나 오프라인 재 upsert 한 번으로 플래그가 세탁되기 때문(QA F-2). **클라이언트 되받기(QA F-4)도 이번 범위에 포함해 함께 닫았다** — `_serverAdjustedKeys`(`distance_meters`·`moving_seconds`)를 신설해 **보내고 또 받으며**, `client_reported` 도 함께 내려와 로컬 `summaryJson` 에 남는다(§7.2). **잔여**(항목 자체는 해소, 아래 2건만 추적): ① **F-7 — 신호등 정지(앵커 변위)를 서버가 세지 못해 서버 값이 체계적으로 짧게 나올 수 있다.** 20% 임계가 대부분 흡수하지만, 이제 원격에 적용됐으므로 **실측(산출물 §6 진단 쿼리)을 실행**해 편향을 확인해야 하고, 확인되면 **서버 재계산에 정지 구간 보정을 넣는 별도 후속**이 필요하다 ② `client_reported` 를 `RunRecord` 필드로 승격 + 상세 화면 표시(flutter-ui 후속) | 잔여 ①: 실측(산출물 §6) 후 backend-engineer / 잔여 ②: flutter-ui-designer |
...
| 33 | **`evaluate_badge_condition` 앵커 치환 패치 2건은 회귀 가드(마이그레이션 63)로만 보호된다.** 함수 전문을 마이그레이션마다 실어 관리하거나 `season_weekly_rank_lte` 분기를 작은 함수로 분해하면 가드가 불필요해진다 — 이 함수를 다음에 실질적으로 손댈 때 함께 처리 | backend-engineer, 이 함수 다음 변경 시 |
| ~~22~~ | ~~주석의 뱃지 SVG 종수가 158종으로 적혀 있으나 정본(`docs/badge-catalog.csv`)은 현재 146개다~~ → **해소(2026-09-03).** `share_card_renderer.dart`는 이전 라운드에서 이미 146종으로 정정돼 있었고, 같은 stale 숫자가 남아 있던 나머지 4곳을 이번에 함께 정정했다 — `core/theme/app_tokens.dart`(158종 카탈로그 → 146종), `features/gamification/domain/badge_condition.dart`(158종 → 146종), `models/badge.dart`(158종 / 16카테고리 → 146종 / 14카테고리), `models/enums.dart`(16개 카테고리 · 158종 → 14개 카테고리 · 146종). 정본 실측: CSV 데이터 행 146개 · `category` 고유값 14개(= `BadgeCategory` enum 멤버 수와 일치). 판정 로직에 쓰이는 숫자가 아니라 서술뿐이므로 동작 변화 없음. 마이그레이션 25·26의 주석("158종 / 16카테고리")은 **적용 완료된 과거 마이그레이션의 기록이라 손대지 않았다** | — |
