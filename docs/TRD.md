...
| v0.23 | **R-12 회귀 가드**(2026-09-01, 마이그레이션 62, 원격 적용 완료). 마이그레이션 57·61 이 `evaluate_badge_condition`(24KB 디스패처)을 **전문 재작성 없이 문자열 앵커 치환**으로 패치했다(57-5 `and le.finalized_at is not null`, 61-3 `runnit.badge_season` GUC). 이후 이 함수를 전문 재정의하는 마이그레이션이 두 조각을 다시 넣지 않으면 에러 없이 사라져 TRD #23·#30 이 부활한다. 62 는 라이브 함수 정의를 `pg_get_functiondef` 로 덤프해 두 문자열의 존재를 검사하고 없으면 `raise exception` 하는 `DO` 블록 — **이후 마이그레이션에서도 유지**한다. 근본 해법(전문 관리 또는 분기 함수 분해)은 이 함수를 다음에 실질적으로 손댈 때. QA `_workspace/20260901_qa_short-week-optionb.md` §3 P-15 |
...
| 33 | **`evaluate_badge_condition` 앵커 치환 패치 2건은 회귀 가드(마이그레이션 62)로만 보호된다.** 함수 전문을 마이그레이션마다 실어 관리하거나 `season_weekly_rank_lte` 분기를 작은 함수로 분해하면 가드가 불필요해진다 — 이 함수를 다음에 실질적으로 손댈 때 함께 처리 | backend-engineer, 이 함수 다음 변경 시 |
