...
| 근거 문서 | [`docs/PRD.md`](./PRD.md) v1.5 (확정), 실제 구현(`lib/`, `supabase/migrations/00~64`) |
| 하위 문서 | [`docs/TRD.md`](./TRD.md) — 이 문서의 결정을 구현 가능한 스펙으로 세분화 |

**변경 이력**
| 버전 | 변경 내용 |
|------|----------|
| v0.21 | **웨어러블 라운드 표기 P1 → Phase 4 통일**(2026-09-03, PRD v1.10 동반). §1.2 범위의 "P1(웨어러블 연동)"을 "Phase 4(웨어러블 연동)"로 정정 — 데이터 모델 선수용 원칙은 불변. §6.3은 이미 "PRD §11 Phase 4"라 정합. |
| v0.20 | **§9 업로드 인플라이트 가드 + 재시도 상한**(2026-09-03, TRD §14 #29 / L-2 해소). `save()`의 백그라운드 업로드와 `RunSyncCoordinator.syncPending()`이 같은 행을 동시에 올리던 경합을 `LocalRunRepository._inFlight` 공유 set 하나로 닫았다. 무한 재시도는 백오프가 아니라 **횟수 상한**(`maxSyncAttempts = 10`, drift 스키마 v1→v2 `sync_attempts`·`last_sync_attempt_at`)으로 끊는다 — 재시도 *간격*은 이미 코디네이터의 4신호가 정하고 있기 때문. 함께: 서버 샘플 기반 거리 재계산(마이그레이션 64, TRD §7.1 / §14 #27) — **2026-09-03 원격 적용 완료**(Supabase `xwtbwexcofcgmbvktwdo`)라 §13 마이그레이션 범위도 00~64로 넓어졌다. 그 재계산 값을 클라이언트가 되받는 경로(`_serverAdjustedKeys`, TRD §7.2 / QA F-4)도 §9에 반영 |
...
| v0.15 | **QA C-1 수정 — 시즌 스냅샷 RLS 반전**(2026-09-01, 마이그레이션 59, 원격 적용 완료). 58의 `season_snapshots_select_visible`가 `season_histories`를 **인라인 서브쿼리**로 조회했는데, RLS 정책식 안의 서브쿼리는 참조 테이블의 RLS를 그대로 받는다 — `season_histories_select_visible`가 이미 타인의 무효 행을 감추므로 `not exists`가 항상 참이 되어 **무효 시즌 스냅샷이 전원에게 공개**됐다(라이브 재현 `snapshot_visible_to_other=1`). SECURITY DEFINER 헬퍼 `_season_history_voided()`로 그 평가를 RLS 밖으로 빼 수정. §5.5 표의 "동일 공개 범위" 서술이 **정책 형태의 차이를 가렸다**는 점을 명시하고 되돌림 금지 경고를 붙였다 |
| v0.14 | **주간 랭킹 확정 배치 + 시즌 랭킹 스냅샷**(2026-09-01, 마이그레이션 57·58, 원격 적용 완료). (1) §5.3에 확정 배치 서술 추가 — 5분 주기 갱신(주중 신선도) 위에 월요일 KST 00:10 `finalize_weekly_ranking()`을 얹어 "확정"을 명시적 상태(`leaderboard_entries.finalized_at` + `weekly_ranking_finalizations` 원장)로 만들고, 그 시점에 RK-06 뱃지를 지급한다. 진행 중인 주의 일시적 1위로 영구 뱃지가 나가던 경로가 닫혔다(TRD §14 #12·#23 해소). (2) §5.3·§13-4 — `season_leaderboard_snapshots` 신설, `recompute_season_tier`의 `season_histories` 마감과 같은 트랜잭션에서 티어별 시즌 랭킹을 적재(PRD RK-10). 첫 적재는 2026-09-30 시즌 마감. (3) §5.5 RLS 표에 두 테이블 추가. (4) **§13-1 stale 정정** — "마이그레이션 51 원격 적용만 남음"은 사실이 아니었다(51은 2026-08-31 적용 완료, HI-07은 서버·클라 모두 끝). §13 도입부의 마이그레이션 범위도 00~48 → 00~58 |
...
### 1.2 범위
- **P0(MVP) 중심**으로 설계하되, Phase 4(웨어러블 연동)는 데이터 모델 레벨에서 처음부터 수용한다 (PRD §5.7 근거 — 나중에 스키마 마이그레이션 없이 붙는다). *(웨어러블 우선순위는 PRD v1.10에서 P1 → Phase 4로 통일됨.)*
...
- Supabase가 **단일 백엔드**다. 별도 마이크로서비스도, **Deno Edge Function도 두지 않는다.** 클라이언트는 `supabase_flutter`로 `runs` 등에 직접 upsert하고, 서버 검증·티어/랭킹/뱃지/XP 판정은 전부 **Postgres 트리거·함수**(마이그레이션 00~64)가 같은 트랜잭션 또는 `pg_cron` 배치로 처리한다 — 1인 개발 기준 운영 복잡도를 최소화하기 위한 선택. (이 문서 곳곳에 남아 있는 "Edge Function" 서술은 초기 설계 표현이며, 실제 구현 위치는 트리거/함수다.)
...
| 구성 요소 | 역할 |
|---|---|
| **Postgres** | 단일 진실 원천 — `runs`/`profiles`/`badges`/`user_badges`/`leaderboard_entries`(+뷰 `leaderboard_entries_active`)/`season_histories`/`tier_change_history`/`weekly_ranking_finalizations`/`season_leaderboard_snapshots`/`challenges` 등 (마이그레이션 00~64) |
...
1. **1단계(현재)**: `leaderboard_entries` 테이블(마이그레이션 03)을 `refresh_all_leaderboards()` + `pg_cron` 5분 주기로 갱신. `period='weekly', metric='distance', scope='global', tier=<4단계>`로 티어별 4개 보드가 유지된다. 지난 주차 행은 삭제하지 않고 누적 보존.
   - **주간 확정 배치(마이그레이션 57, 2026-09-01)**: 5분 주기 갱신은 *주중 신선도*를 담당하고, 그 위에 **매주 월요일 KST 00:10**(`10 15 * * 0` UTC, job `runnit-weekly-finalize`) `finalize_weekly_ranking()`이 얹힌다 — 방금 끝난 주를 티어별로 최종 재집계하고 `leaderboard_entries.finalized_at` 도장 + `weekly_ranking_finalizations(주×티어)` 원장에 확정 사실을 남긴 뒤, 그 자리에서 RK-06 주간 상위권 뱃지를 지급한다. **"확정"이 명시적 상태가 되면서 진행 중인 주의 일시적 1위로 영구 뱃지가 나가던 경로가 닫혔다**(`season_weekly_rank_lte` 판정에 `finalized_at is not null` 필터). NT-06 알림만은 §5.6의 방해 금지(KST 08~22)를 지켜 같은 날 08:00 KST job(`runnit-weekly-rank-badge-notify`)이 발행한다.
   - **시즌 말 단축 주간(마이그레이션 61 — PRD §8.5 구현)**: 시즌 경계는 분기 1일, 주 경계는 월요일이라 거의 매 시즌 마지막 주가 두 시즌에 걸친다. `leaderboard_period_bounds('weekly')`가 달력 주를 **앵커가 속한 시즌으로 클램프**해 조각 2개로 나눈다 — `period_start`가 달라 **스키마 변경 없이** 별개 기간이 되고, `week_id_at()`이 걸친 주에만 `@{season}`을 붙여 알림 dedupe 키 충돌을 막는다. 확정 배치도 "직전 1개"가 아니라 **끝났는데 미확정인 모든 주간 기간(14일 되짚기)**을 확정하고, 시즌 롤오버(`reset_stale_seasons`)가 리셋 루프 **전에** 확정 배치를 불러 구시즌 조각을 시즌 마감과 같은 순간에 확정한다. 함께 **뱃지 판정의 시즌**이 "지금"에서 **판정 중인 뱃지 인스턴스의 `badges.season_id`**(`runnit.badge_season` GUC)로 바뀌었다 — 이것이 걸친 주 RK-06 미지급(QA C-2)의 진짜 원인이었고, 과거 시즌 인스턴스가 현재 데이터로 재판정되던 잠재 결함도 함께 닫혔다.
   - **시즌 랭킹 스냅샷(마이그레이션 58)**: 주간 보드는 계속 덮어써지므로 *시즌* 랭킹은 별도 영구 테이블 `season_leaderboard_snapshots`에 보존한다. 쓰기는 시즌 경계 배치 한 곳뿐 — `recompute_season_tier`가 `season_histories`를 마감하는 **같은 트랜잭션**에서 티어별 시즌 누적 거리 랭킹을 한 번 적재한다(PRD RK-10 / HI-06).
   - **클라이언트 조회는 `leaderboard_entries_active` 뷰만 읽는다(마이그레이션 62)**: 지난 기간 행을 누적 보존하고 단축 주간이 같은 달력 주를 `period_start`가 다른 두 조각으로 나누므로, 베이스 테이블을 그대로 조회하면 여러 기간 행이 섞여 `rank`가 중복된다. 뷰는 각 행의 `period_start`가 `leaderboard_period_bounds(period, now())`의 활성 기간 시작과 일치하는 행만 노출한다 — KST·시즌 클램프 규칙을 **서버 한 곳**(`leaderboard_period_bounds`)에만 두고 클라이언트가 복제하지 않게 하는 것이 핵심. `security_invoker = true`로 베이스 테이블 RLS를 승계하고, Realtime 구독만은 뷰가 publication에 없으므로 계속 베이스 테이블 `leaderboard_entries`로 한다(재조회는 뷰). `finalized_at`은 뷰 필터에 넣지 않는다 — 진행 중인 주도 화면에 보여야 하고 확정 도장은 뱃지 판정용이다.
...
| `leaderboard_entries` | 전체 select 허용, 쓰기는 `refresh_all_leaderboards()` / `finalize_weekly_ranking()`(둘 다 SECURITY DEFINER)만 |
| `weekly_ranking_finalizations` | 전체 select(`authenticated`+`anon` — 개인정보가 없는 집계 사실), 쓰기 정책 없음 = `finalize_weekly_ranking()`만(마이그레이션 57) |
| `season_leaderboard_snapshots` | `season_histories`와 **공개 범위도 정책 형태도 동일** — `is_voided = false or user_id = auth.uid()` 순수 컬럼 술어, `anon` 차단(마이그레이션 60). 무효 여부를 `is_voided` 컬럼에 **복사해 두어** 교차 테이블 참조가 없다. ⚠️ **`season_histories` 서브쿼리로 되돌리면 차단이 조용히 반전된다** — RLS 정책식 안의 서브쿼리는 참조 테이블의 RLS를 그대로 받으므로, `season_histories`가 이미 감춘 타인의 무효 행이 서브쿼리에도 안 보여 `not exists`가 항상 참이 된다(58의 실제 버그 QA C-1 → 59가 SECURITY DEFINER 헬퍼로 수정 → 60이 헬퍼의 RPC 노출까지 없앰). ⚠️ 사후 무효 판정은 반드시 `set_season_history_voided(user_id, season_id, voided)`로 — 두 테이블을 한 트랜잭션에서 갱신하는 유일한 경로다. 쓰기 정책 없음 = `snapshot_season_leaderboard()`만 |
...
**완료(구현·테스트 존재)**: 데이터 모델(`lib/models/`), Supabase 스키마·트리거(마이그레이션 00~64 — **알림 백엔드 44~48, 주간 랭킹 확정 배치 57, 시즌 랭킹 스냅샷 58, 스냅샷 RLS 수정 59·60, 시즌 말 단축 주간 61, 주간 조회 뷰 `leaderboard_entries_active` 62(+62b grant), `evaluate_badge_condition` 회귀 가드 63, 서버 샘플 기반 거리 재계산 64 포함**. 로컬·원격 전량 적용 완료 — 64는 2026-09-03 적용), GPS 트래킹 코어(스무딩·백그라운드·집계), 오프라인 저장(drift)+동기화, 히스토리 목록·월간 차트, **기록 상세 화면(HI-02 — 경로 지도·1km 랩 테이블·페이스 그래프, `run_detail_page.dart` + `history/domain/lap_splits.dart`)**, 주간 랭킹 + 티어 + Realtime, 뱃지 판정(39 condition_type 전량)·갤러리·레벨·주간 스트릭·XP, 공유 카드(PB)·풀페이지 성취 축하, 카카오 OAuth 로그인, **GPX 내보내기(HI-09 — 클라이언트 전용, `history/domain/gpx_encoder.dart` + `history/data/gpx_export_service.dart`, 상세 화면 오버플로 메뉴)**.

**남은 P0(요약 — 상세는 사용자에게 별도 정리)**:
1. 기록 **수정(제목·메모)**(HI-07) — **서버·클라이언트 모두 완료.** 서버는 마이그레이션 51(`runs_guard`가 터미널 상태 행의 UPDATE를 `title`/`note`로 한정, `runs_delete_own` 정책 제거, 길이 CHECK), 클라이언트는 `RunRepository.updateMeta()` + 상세 화면 편집 바텀시트(`run_meta_edit_sheet.dart`, 2026-08-31). **마이그레이션 51은 2026-08-31 원격 적용 완료 — HI-07은 서버·클라이언트 모두 끝났고 남은 작업이 없다**(2026-09-01 정정: 이 문단은 "51 원격 적용만 남음"으로 stale하게 적혀 있었다). **삭제는 스펙에서 제거됨**(PRD v1.6 — 러닝·뱃지 사용자 삭제 불가). `RunRepository.delete()`는 미완결 세션 폐기 전용으로 의미가 좁혀졌고 서버 DELETE를 더 이상 호출하지 않는다
...
4. ~~역대 시즌 기록 화면(HI-06)~~ — **완료**(2026-08-31). `season_histories` 읽기 전용 리포지토리(`SeasonHistoryRepository` + Supabase 구현, `season_id desc`)·`mySeasonHistoriesProvider`·`SeasonHistoryPage`(`features/history/presentation/`). 진입점은 마이페이지 "역대 시즌"(`/seasons`, 게스트 차단)과 활동 탭 시즌 필. 무효 시즌(`is_voided`)은 숨기지 않고 흐리게+"무효 처리됨" 표시, TI-09(역대 최고 티어)는 목록 상단 요약으로 노출(무효·진행 중 시즌 제외). **남은 것**: 과거 시즌 **전체 랭킹**의 **클라이언트 UI**. ~~스냅샷 테이블 없음~~ → **백엔드 완료(2026-09-01, 마이그레이션 58)**: `season_leaderboard_snapshots(season_id, user_id, tier, rank, season_distance_meters, run_count, moving_seconds, reached_at, participant_count, is_voided, computed_at)`, PK `(season_id, user_id)`. `recompute_season_tier`가 `season_histories` 마감과 **같은 트랜잭션**에서 티어별 시즌 누적 거리 랭킹을 한 번 적재한다(PRD RK-10). 공개 범위·정책 형태 모두 `season_histories_select_visible`와 동일(authenticated 공개, anon 차단, `is_voided` 시즌은 본인만 — 마이그레이션 59·60). 첫 적재는 2026-09-30 시즌 마감(cron `runnit-season-reset-30`). 그리고 타 사용자 프로필의 역대 시즌(AC-03과 함께)
