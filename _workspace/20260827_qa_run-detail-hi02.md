...
## 후속 처리 (오케스트레이터, 2026-08-27)

| # | 처리 | 커밋 |
|---|------|------|
| I-3 | `docs/ARCHITECTURE.md` §13을 HI-02 완료 상태로 갱신 (RunDetailPlaceholder 언급 제거) | b59535d |
| I-4 | `run_tile.dart` "상세 화면 없음" 주석 갱신 | b59535d |
| I-1 | OSM 출처 표기를 `core/widgets/osm_attribution.dart` 공용 위젯으로 승격, 트래킹 지도에도 적용 | b4ee46f |
| I-2 | `findById`가 samples 없이 캐시된 불완전 행을 상세 조회에 내주지 않도록 + 오염 행 자가 치유 | 93b1a91 |
| I-5 | 일시정지가 길었던 러닝(경과−이동 > 60s)에만 "구간 시간은 경과 시간 기준" 한 줄 안내 추가. 랩 계산은 서버(경과 기준)와 계속 일치 | (이 커밋) |
| I-6 | `_timeOrdered`가 순서 뒤바뀐 샘플을 버리지 않고 **정렬** 후 사용(서버 `order by` 정합). 같은 시각 중복만 제거. 셔플 불변성 테스트 추가 | (이 커밋) |

UNVERIFIED 2건(딥링크 실기기 해석, 서버 스플릿 실수치 대조)은 P1 웨어러블 임포트 경로가 생길 때 재검토.
