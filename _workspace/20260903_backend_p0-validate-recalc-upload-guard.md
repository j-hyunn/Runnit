...
### 6.3 1차 실행 결과 (2026-09-03, 프로덕션 `xwtbwexcofcgmbvktwdo`)

**표본이 매우 작다.** `status='completed'` 47건 중 재계산 대상(`activity_type<>'indoor_run'` + 샘플 ≥2)은 **5건뿐**이다. 나머지 42건은 시드/개발 아티팩트(0 샘플 35건 = 평균 13.8km 라운드 시드, 1 샘플 7건 = 거리 0 테스트). 베타 전이라 실사용 도심 러닝 데이터가 없다 — 아래는 참고치이며 F-7 판정은 베타 데이터로 재실행해야 한다.

| id | claimed_m | server_m | delta_pct | would_flag | stationary_seg | removed | 비고 |
|---|---|---|---|---|---|---|---|
| `9e050888` | 766 | 75 | **−90.2%** | ✅ | 1 | **32 / 40** | 구간 평균속도 7.06 m/s(25.4km/h)·최대 8.38 m/s. 대부분 구간이 25km/h 초과 — 정상 야외 러닝 측정이 아니다(개발 테스트 또는 GPS 글리치). **#27이 잡으라고 만든 바로 그 케이스.** 라이브 행은 백필 안 함이라 `is_flagged=false` 유지 |
| `ba2b5a9b` | 165 | 167 | +1.36% | — | 0 | 0 | 클램프(서버>클라) → 무변화 |
| `ec370c73` | 233 | 236 | +0.91% | — | 0 | 0 | 클램프 → 무변화 |
| `6d7afcd5` | 502 | 505 | +0.42% | — | 1 | 0 | 클램프 → 무변화. 정지구간 1개(20s)지만 거리 영향 없음 |
| `5620c9f6` | 130 | 132 | +1.63% | — | 0 | 0 | 클램프 → 무변화 |

분포 요약: `runs=5 mean_pct=−17.17 p05=−72.07 p50=+0.91 p95=+1.58 p99=+1.62`. `p05/mean`의 음수는 전부 `9e050888` 한 건이 만든 것이고, 나머지 4건은 전부 **+0.4~1.6%**(서버가 오히려 미세하게 크게 나와 클램프로 무변화).

**F-7 신호 없음 (현 표본 한정).** 정상 4건의 `stationary_segments`가 0~1, `delta_pct`가 +0에 붙어 있다 — 신호등 앵커 변위로 서버가 체계적으로 짧게 잡는 편향은 이 표본에서 나타나지 않는다. `v_flag_shrink_ratio = 0.8` 도 현행 유지가 타당하다(정상 기록 오탐 0건, 25km/h 이상치 1건만 적중).

**다음**: 클로즈드 베타로 도심 장거리 러닝(신호등·횡단보도 다수)이 20건 이상 쌓이면 §6.1/§6.2를 재실행한다. 그때 `mean_pct`가 −3% 이하로 내려가거나 `stationary_segments`와 `delta_pct`가 상관되면 정지구간 보정 후속에 착수.
