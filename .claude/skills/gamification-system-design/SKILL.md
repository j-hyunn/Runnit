...
- **기간 분할**: 주간/월간/전체 리더보드를 별도로 관리 — 신규 사용자가 전체 랭킹에서 항상 하위에 머무르면 이탈 요인이 되므로 주간 랭킹으로 재도전 기회를 준다.
- **동점 처리**: 반드시 명시적 규칙을 정한다. 예: 점수 동일 시 해당 점수를 먼저 달성한 시각(timestamp) 기준 오름차순.
- **그룹 스코프**: 전체/지역/크루 단위 랭킹을 지원할 경우, 랭킹 계산은 스코프별로 독립적으로 캐시한다(§ backend 스킬의 집계 전략 참조).
...
기간 한정 목표(예: "이번 달 50km 달성") 또는 크루 대항전(팀 합산 거리 경쟁)을 설계할 때는:
